...
function CreateTeacherUser(props) {
  ...
  const onSpecificPage = onPage.startsWith(
    "/admin/users/overview/teacher/create/user/specific"
  );
  // console.log(onPage);
  ...
  const getRole = async () => {
    try {
      const response = await fetch(`${apiURL}/api/role/loadn/Teacher`, {
        headers: header,
      });
      const data = await response.json();
  ...
  return (
    <RowWrapper className="w-100p bw-none justify-between gap-20 align-start">
      ...
      >
        <Label text="Enter Teacher's user's information" />
        ...
        <form onSubmit={handleSubmit}>
          ...
          <ColumnWrapper style={{ border: "none" }}>
            ...
            <Select
              placeholder="Select User's Address"
