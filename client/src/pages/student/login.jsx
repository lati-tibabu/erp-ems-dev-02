...
function StudentLogin() {
  ...
  const [userDataInfo, setUserDataInfo] = useState({ user: {}, student: {}, school: {} });
        ...
        if (roleData.role_name === 'Student') {
          ...
          const getUserInfo = async (u) => {
            try {
              ...
              const studentResponse = await fetch(`${apiURL}/api/student/loadu/${userData.user_id}`, {
                method:'GET', 
                headers: header });
              const studentData = await studentResponse.json();

              const schoolResponse = await fetch(`${apiURL}/api/school/load/${studentData.school_id}`, { headers: header });
              const schoolData = await schoolResponse.json();

              const classResponse = await fetch(`${apiURL}/api/class/load/${studentData.class_id}`, { headers: header });
              const classData = await classResponse.json();

              const userInfo = { user: userData, student: studentData, school: schoolData, class: classData };
              ...
              <form onSubmit={handleSubmit}>
                ...
                {!loggingIn
                    ...
                    :<div className='font-xs flex-row align-center w-50p mt-20px'>
                          <img className='p-0 w-30px h-30px' style={{filter: 'invert(100%)', mixBlendMode:'darken'}} src={amalie_loading}/>
                              Logging in...
                      </div>
                }
              </form>
